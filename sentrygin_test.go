...
func TestIntegration(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			WantStatus:  200,
			Body:        "payload",
			Handler: func(c *jin.Context, hub *sentry.Hub) {
		...
		},
		{
			...
			Handler: func(c *jin.Context, hub *sentry.Hub) {
				...
				c.Writer.WriteHeader(http.StatusOK)
				_, _ = c.Writer.Write([]byte("{\"status\":\"get\"}"))
			},
			WantTransaction: &sentry.Event{
				Level:       sentry.LevelInfo,
				Type:        "transaction",
		...
		},
		{
			...
			Handler: func(c *jin.Context) {
				...
				_, _ = c.Writer.Write([]byte("{\"status\":\"bad_request\"}"))
			},
			WantTransaction: &sentry.Event{
				Level:       sentry.LevelInfo,
				Type:        "transaction",
